...
-- trimmed == Talse means no terms like 0 * x^n
data Poly a where
    ListPoly ::
        { trimmed    :: !Bool
...
ppListPoly    :: ( Show a, Num a, Eq a ) => Endianness -> [a] -> String
ppVectorPoly  :: ( Show a, Num a, Eq a ) => Endianness -> V.Vector a -> String
ppUVectorPoly :: ( Show a, Num a, Eq a, UV.Unbox a ) => Endianness -> UV.Vector a -> String 

ppListPoly LE coefficients = (show coefficients)
ppListPoly BE coefficients = (show $ reverse coefficients)

ppVectorPoly e coefficients = ppListPoly e (V.toList coefficients)
ppUVectorPoly e coefficients = ppListPoly e (UV.toList coefficients)

instance ( Show a, Eq a, Num a ) => Show (Poly a) where
    show (ListPoly _ e c) = ppListPoly e c
    show (VectorPoly _ e c) = ppVectorPoly e c
    show (UVectorPoly _ e c) = ppUVectorPoly e c 
